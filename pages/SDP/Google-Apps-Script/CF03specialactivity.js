function ConditionalFormattingSpecialActivityTab() {
 
    /**
    * ConditionalFormattingSpecialActivityTab Function.
    * Conditionally formats the SpecialActivity tab.
    */
    
    // Add an alert to tell the user that the sheet is being formatted.
    var SpreadsheetUI = SpreadsheetApp.getUi();
    var ProperTabName = "Special Activity";
    var TabName = "specialactivity";
    ...
    for (i = 1; i <= SwimlaneArraySize; i++) {
      // Activity Formatting. Highlights the activity name, start, duration and task name for all swimlanes.
       ConditionalFormatRules.push(SpreadsheetApp.newConditionalFormatRule()
          .setRanges([CurrentSpreadsheet.getRange(FirstDataColumnLetter + FirstRow + ':' + nameColumnLetter + LastRow),CurrentSpreadsheet.getRange(longNameColumnLetter + FirstRow + ':' + LastDataColumnLetter + LastRow),CurrentSpreadsheet.getRange(TaskColumnLetter + FirstRow + ':' + TaskColumnLetter + LastRow)])
