function ConditionalFormattingTaskHasDeliverableTab() {
 
    /**
    * ConditionalFormattingTaskHasDeliverableTab Function.
    * Conditionally formats the Task Has Deliverable tab.
    */
    
    // Add an alert to tell the user that the sheet is being formatted.
    var SpreadsheetUI = SpreadsheetApp.getUi();
    var ProperTabName = "Task Has Deliverable";
    var TabName = "task_has_deliverable";
    ...
    // Set up a number of column references to be used below. Note: Variables used across "task_has..." and "activity_has..." tabs hence the naming.
    var idColumnLetter = "A";
    var NumberColumnLetter = "B";
    var deliverableIDColumnLetter = "C";
    var taskORactivityIDColumnLetter = "D";
    var ActiveColumnLetter = "F";
    var LastDataColumnLetter = "H";
    var LineStatusColumnLetter = "J";
    var RemarkColumnLetter = "K";
    var DeliverableDescriptionColumnLetter = "M";
    var TaskORActivityColumnLetter = "N";
    var OwnerColumnLetter = "O";
    var ownerIDColumnLetter = "P";
    var DeliverableTaskCheckColumnLetter = "S";
    var DeliverableActivityCheckColumnLetter = "T";
    var ErrorCheckColumnLetter = "V";
    var FirstCheckColumnLetter = "X";
    var LastCheckColumnLetter = "AF";
    ...
    CurrentSpreadsheet.getRange(deliverableIDColumnLetter + FirstRow + ':' + taskORactivityIDColumnLetter + LastRow).setBackground('#efefef')
    .setFontColor(null)
    .setFontStyle('italic');
    CurrentSpreadsheet.getRange(OwnerColumnLetter + FirstRow + ':' + ownerIDColumnLetter + LastRow).setBackground('#efefef')
    .setFontColor(null)
    .setFontStyle('italic');
    CurrentSpreadsheet.getRange(DeliverableTaskCheckColumnLetter + FirstRow + ':' + DeliverableActivityCheckColumnLetter + LastRow).setBackground('#efefef')
    .setFontColor(null)
    .setFontStyle('italic');
    ...
    Protection = CurrentSpreadsheet.getRange(deliverableIDColumnLetter + FirstRow + ':' + taskORactivityIDColumnLetter + LastRow).protect();  
    Protection.removeEditors(UserEmailString.toString().split(","));
    Protection = CurrentSpreadsheet.getRange(OwnerColumnLetter + FirstRow + ':' + ownerIDColumnLetter + LastRow).protect();
    Protection.removeEditors(UserEmailString.toString().split(","));
    ...
    ConditionalFormatRules.push(SpreadsheetApp.newConditionalFormatRule()
       .setRanges([CurrentSpreadsheet.getRange(idColumnLetter + FirstRow + ':' + LastDataColumnLetter + LastRow),CurrentSpreadsheet.getRange(DeliverableDescriptionColumnLetter + FirstRow + ':' + TaskORActivityColumnLetter + LastRow)])             
    ...
    ConditionalFormatRules.push(SpreadsheetApp.newConditionalFormatRule()
       .setRanges([CurrentSpreadsheet.getRange(NumberColumnLetter + FirstRow + ':' + LastDataColumnLetter + LastRow)])
    ...
    for (i = 1; i <= SwimlaneArraySize; i++) {
      // Activity Formatting. Highlights the Activity for all swimlanes.
       ConditionalFormatRules.push(SpreadsheetApp.newConditionalFormatRule()
          .setRanges([CurrentSpreadsheet.getRange(TaskORActivityColumnLetter + FirstRow + ':' + TaskORActivityColumnLetter + LastRow)])
