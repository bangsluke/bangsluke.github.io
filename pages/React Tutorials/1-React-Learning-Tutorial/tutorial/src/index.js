...
// Implicit Return
const Welcome = () => (
  <h4>Hello Luke, this is your first React Tutorial talking.</h4>
);
...
const Message = () => {
  return (
    <div>
      ...
      <ul>
        className instead of class, e.g. "className='left'" instead of
        "class='left'" in normal HTML.
      </ul>
      <ul>
        Close every element, e.g. have to close the img tag unlike in normal
        HTML.
      </ul>
      <ul>
        Formatting: It's best to have brackets around your return statement as
        per this example.
      </ul>
    </div>
  );
...
const Car5Details = {
  ImageURL:
    "https://tse4.mm.bing.net/th?id=OIP.Notu4f0sx6bjekTzOk1oeQHaE6&pid=Api",
  Model: "Gallardo",
...
const Car6Details = {
  ImageURL:
    "https://tse3.mm.bing.net/th?id=OIP.s1p_7HB5c-FvgfyPS08rxAHaEK&pid=Api",
  Model: "Roadster",
  Manufacturer: "Tesla",
};
    ...
    <React.Fragment>
      ...
      <section className="carList">
        ...
        <Car4
          Model="Model 3"
          Manufacturer="Tesla"
          ImageURL="https://cdcssl.ibsrv.net/autodata/images/?IMG=USD10TSC032B01308.JPG&WIDTH=1200"
        />
        <Car5
          Model={Car5Details.Model}
          Manufacturer={Car5Details.Manufacturer}
          ImageURL={Car5Details.ImageURL}
        />
        <Car6
          Model={Car6Details.Model}
          Manufacturer={Car6Details.Manufacturer}
          ImageURL={Car6Details.ImageURL}
        >
...
const Image = () => (
  <img
    src="https://s1.cdn.autoevolution.com/images/gallery/HONDA-Civic-5-Doors-888_32.jpg"
    alt=""
  ></img>
);
const ModelName = () => <h3>Civic 2005</h3>;
const Manufacturer = () => (
  <h4 style={{ color: "#617232", fontSize: "0.75rem", margin: "0.25rem" }}>
    Honda
  </h4>
); // Note you can have inline style as shown.
...
const Car3 = () => {
  ...
  return (
    <article className="car">
      <img
        src="http://s3.caradvice.com.au/wp-content/uploads/2013/06/Mazda-3-hatch-71.jpg"
        alt=""
      ></img>
      <h3>{Car3Model}</h3>
...
const Car6 = ({ ImageURL, Model, Manufacturer, children }) => {
  // The destructuring line has moved to here, I don't need to call each below variable as "props.xxx"
...
const CarListArray = [
  {
    id: 1,
    ImageURL:
      "http://s3.caradvice.com.au/wp-content/uploads/2013/06/Mazda-3-hatch-71.jpg",
    Model: "Mazda 3",
    Manufacturer: "Mazda",
  },
  {
...
const CarComponentWithEvents = ({ ImageURL, Model, Manufacturer }) => {
  ...
  // Simple event.
  const clickHandler = () => {
    // Set up the eventHandler as a reference. Can also do as inline.
    alert("Hello");
  };

  // The below set up allows us to pass a variable to the handler event.
  const clickHandlerComplexExample = (Manufacturer) => {
    // Set up the eventHandler as a reference.
    alert(Manufacturer);
  };
  ...
  return (
    <article className="refactoredCarWithEvents" onMouseOver={logCarComponent}>
      ...
      {/* Example here calls the reference function above. */}
      <button type="button" onClick={() => alert(Model)}>
        Get Model
      </button>{" "}
      {/* Example here uses an inline function. */}
      <button
        type="button"
        onClick={() => clickHandlerComplexExample(Manufacturer)}
      >
        Get Manufacturer
      </button>{" "}
      {/* Example here calls a more complex reference function above, showing how to pass an argument combining an arrow function with a reference function. */}
      <button type="button" onClick={clickHandlerWhere}>
        Where?
      </button>{" "}
      {/* Example here calls the reference function above. */}
    </article>
